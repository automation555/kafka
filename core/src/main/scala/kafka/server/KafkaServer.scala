...
class KafkaServer(
  ...
  override def startup(): Unit = {
    try {
      ...
      if (canStartup) {
        ...
        if (config.interBrokerProtocolVersion >= kafka.api.KAFKA_2_8_IV2) {
  ...
  protected def createReplicaManager(isShuttingDown: AtomicBoolean): ReplicaManager = {
    new ReplicaManager(config, metrics, time, Some(zkClient), kafkaScheduler, logManager, isShuttingDown, quotaManagers,
      brokerTopicStats, metadataCache, logDirFailureChannel, alterIsrManager, configRepository, logDirEventManager = logDirEventManager)
  }
