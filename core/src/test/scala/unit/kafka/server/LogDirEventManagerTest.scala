...
class LogDirEventManagerTest {
  ...
  def testBuildRequest(): Unit = {
    val capture = EasyMock.newCapture[AbstractRequest.Builder[AlterReplicaStateRequest]]()
  ...
  def testPartialSuccess(): Unit = {
    ...
    val requestCapture = EasyMock.newCapture[AbstractRequest.Builder[AlterReplicaStateRequest]]()
    EasyMock.reset(brokerToController)
    EasyMock.expect(brokerToController.sendRequest(EasyMock.capture(requestCapture), EasyMock.anyObject())).once()
    EasyMock.replay(brokerToController)
